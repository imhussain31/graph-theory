For a simpler approach, the primary focus is to compute the shortest path in an undirected graph using BFS without additional complexities. Here’s an **easier solution** where we focus directly on finding the path, avoiding unnecessary arrays or extra calculations:

### Simplified Code for Shortest Path
...
1. **Path Tracked Directly**:  
   - Instead of using a `parent` array, paths are explicitly tracked by appending nodes to the current path.

2. **Efficient Use of BFS**:  
   - BFS guarantees the shortest path in an unweighted graph, so we simply extend paths until the destination is found.

3. **Stopping Condition**:  
   - The traversal stops as soon as the destination is reached, reducing unnecessary iterations.

4. **Clearer Code**:  
   - The algorithm focuses only on the essentials: BFS traversal and path tracking.

### Input/Output for the Given Test Case
...
### Why It’s Easier
- No need to manually reconstruct the path using a `parent` array.
- Avoids extra complexity with distances for intermediate nodes unless necessary.
- Focuses on BFS with immediate results.
